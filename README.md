...
Время измеряется как среднее арифметическое среди 5 измерений. Сборка осуществляется с флагом оптимизации: __-O1__.

## Первая версия ##
 Для проверки скорости работы хеш-таблицы загрузим каждое второе слово из первого томы "Войны и мира", затем осуществим 40000000 операций поиска различных слов из этого романа. Как тех слов, которые добавлены в таблицу, так и тех, которые мы не добавили

 |__Время:__                                                              | 6 ± 0.03 сек.|

## Нулевая оптимизация ##
 Установим `#define NDEBUG` перед подключением `#include <assert.h>` и уберём `#define ON_LIST_CHECKING` из файла List.h, чтобы отключить проверки.

 |                                                                        |                               |
 |:-----------------------------------------------------------------------|:-----------------------------:|
 |__Время:__                                                              | 5.95 ± 0.03 сек.
 |__Абсолютный коэффициент ускорения:__                                   | 1.008

 __Замечание:__  Отключение проверок в этом случае не дало прироста производительности, но такой шаг необходимо делать в общем случае, когда проверки могут быть более затратными.
